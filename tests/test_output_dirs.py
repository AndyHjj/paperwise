...
def test_new_arxiv_paper_uses_stable_id_directory(tmp_path: Path) -> None:
    ...
    _write_meta(first.path, arxiv_id="2502.14802v1", title="Original title")
    ...
    assert first.path == tmp_path / "2502_14802_Original_title"
    assert second.path == first.path
    assert first.identity == "arxiv:2502.14802"


def test_richer_existing_directory_wins_and_duplicates_are_reported(
    tmp_path: Path,
) -> None:
    ...
    assert resolution.path == old_title_dir
    assert resolution.duplicate_dirs == (canonical,)


def test_preferred_migration_path_contains_id_and_readable_title(
    tmp_path: Path,
) -> None:
    path = preferred_paper_path(
        tmp_path,
        arxiv_id="2502.14802v3",
        title="From RAG to Memory: Non-Parametric Continual Learning",
    )

    assert path.name == (
        "2502_14802_From_RAG_to_Memory_Non-Parametric_Continual_Learning"
    )


def test_long_preferred_path_does_not_cut_the_last_title_word(
    tmp_path: Path,
) -> None:
    path = preferred_paper_path(
        tmp_path,
    ...
    assert not path.name.endswith("_Pro")
